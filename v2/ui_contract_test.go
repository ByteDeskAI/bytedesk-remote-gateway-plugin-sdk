...
func TestPublishedWebAppsBrowserArtifactsComeFromPinnedCommonSDKV2(t *testing.T) {
	moduleDir, err := exec.Command("go", "list", "-m", "-f", "{{.Dir}}", "github.com/ByteDeskAI/bytedesk-sdk-dependencies/v2").Output()
	if err != nil {
		t.Fatal(err)
	}
	common := filepath.Join(strings.TrimSpace(string(moduleDir)), "webapps", "typescript")
	published := filepath.Join(moduleRoot(t), "..", "ui", "v2", "webapps")
	for _, name := range []string{"contracts.d.ts", "validators.js", "descriptors.js"} {
		t.Run(name, func(t *testing.T) {
			want, err := os.ReadFile(filepath.Join(common, name))
			if err != nil {
				t.Fatal(err)
			...
			if !bytes.Equal(got, want) {
				t.Fatalf("ui/v2/webapps/%s drifted from the pinned common SDK v2 generator output", name)
			}
		})
	}
}
